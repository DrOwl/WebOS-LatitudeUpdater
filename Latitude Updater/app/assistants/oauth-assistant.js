function OauthAssistant(oauthConfig) {
	...
	this.requestParams = oauthConfig.requestParams;
	this.authorizeParams = oauthConfig.authorizeParams;
}
...
OauthAssistant.prototype.titleChanged = function(event) {
    ...
    if (!this.exchangingToken && (responseVars[0] == this.callbackURL + '/' || responseVars[0] == this.callbackURL)) 
	{
		...
		if (result != null) 
		{
			...
			for (i = 0; i < 2; i++) 
			{
				result = params[i].match(/oauth_token=*/g);
				if (result != null) 
				{
					token = OAuth.decodePercent(params[i].replace("oauth_token=", ""));
				}
				else 
				{
					this.oauth_verifier = OAuth.decodePercent(params[i].replace("oauth_verifier=", ""));
				}
			}
...
OauthAssistant.prototype.signHeader = function (params){
	...
	if (!this.exchangingToken) 
	{
		...
		{
			this.message.parameters.push([prop, this.requestParams[prop]]);
		}
	}
...
OauthAssistant.prototype.requestToken = function (){
	...
	onComplete:function(response)
	{
		...
		for (var prop in this.authorizeParams)
		{
			auth_url += "&"+prop+"="+OAuth.percentEncode(this.authorizeParams[prop]);
		}
